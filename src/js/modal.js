 ...
 * https://github.com/nico3333fr/van11y-accessible-modal-window-aria/issues/9
 */
;(doc => {
  "use strict"
  ...
  const createOverlay = config => {
    ...
    let overlayBackgroundEnabled =
      config.backgroundEnabled === "disabled" ? "disabled" : "enabled"
  ...
  const createModal = config => {
    ...
    let buttonCloseClassName =
      config.modalPrefixClass + MODAL_BUTTON_CLASS_SUFFIX
    ...
    let describedById =
      config.modalDescribedById !== ""
        ? `${ATTR_DESCRIBEDBY}="${config.modalDescribedById}"`
        : ""
      ...
      ;["click", "keydown"].forEach(eventName => {
        doc.body.addEventListener(
          ...
          e => {
            ...
            if (
              (hasClass(e.target, MODAL_JS_CLASS) === true ||
                parentModalLauncher !== "") &&
              eventName === "click"
            ) {
              let body = doc.querySelector("body")
              ...
              let modalDescribedById =
                modalLauncher.hasAttribute(MODAL_DESCRIBEDBY_ID_ATTR) === true
              ...
              let backgroundEnabled =
                modalLauncher.hasAttribute(MODAL_DATA_BACKGROUND_ATTR) === true
            ...
            if (findById(MODAL_JS_ID) && eventName === "keydown") {
              ...
              let $listFocusables = [].slice.call(
                modal.querySelectorAll(FOCUSABLE_ELEMENTS_STRING)
              )
              ...
              if (e.keyCode === 9 && $listFocusables.indexOf(e.target) >= 0) {
                ...
                if (e.shiftKey) {
                  ...
                  if (
                    e.target === $listFocusables[$listFocusables.length - 1]
                  ) {
  ...
  const onLoad = () => {
    ...
    if (modalConfig) {
      let setModal = {
        ...
        modalDescribedById: modalConfig.getAttribute(
